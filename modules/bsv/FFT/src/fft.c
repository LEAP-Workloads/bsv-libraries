...
int checkRealResult(int index, int result) {
  ...
  #endif
  if((realResult[index] - ((float)2048)/(1<<fracSize) < result/((double) (1<<fracSize))) &&
     (realResult[index] + ((float)2048)/(1<<fracSize) > result/((double) (1<<fracSize)))) {
    printf("okay\n");
    return 0;
  } else {
    printf("error\n");
...
int checkImagResult(int index, int result ) {
  ...
  #ifdef DEBUG
  printf("Imag result[%d]: %f,  expect %f max delta: %f ", index, result/((double) (1<<fracSize)), imagResult[index],
  ((float)2048)/(1<<fracSize));  
  #endif
  if((imagResult[index] - ((float)2048)/(1<<fracSize) < result/((double) (1<<fracSize))) &&
     (imagResult[index] + ((float)2048)/(1<<fracSize) > result/((double) (1<<fracSize)))) {
    printf("okay\n");
    return 0;
  }
  else {
